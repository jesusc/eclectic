...
public class AntRewriteTask extends MatchingTask {
	...
	/**
	 * Directory to which the transformed files will be written. This can be the
	...
	/**
	 * Directory from which the input files are read. This and the inherited
	 * {@link MatchingTask} forms an implicit {@link FileSet}.
	 */
	 ...
	 * 
	 * <p>
	 * This option is unpublished, because in a long run we'll likely to just
	 * focus on one transformer and get rid of the other (and this option will
	 * be removed then.)
	 * 
	 * @param name
	...
	private void checkDir(final File pDir, final String pDescription) {
		if (pDir == null) {
			throw new BuildException("no " + pDescription
					+ " directory is specified", getLocation());
		}
		if (!pDir.exists()) {
			throw new BuildException(pDescription + " directory \"" + pDir
					+ "\" does not exist", getLocation());
		}
		if (!pDir.isDirectory()) {
			throw new BuildException(pDescription + " directory \"" + pDir
					+ "\" is not a directory", getLocation());
		}
	}
		...
		try {
			for (int i = 0; i < fileNames.length; i++) {
				...
				if (source.lastModified() < destination.lastModified()) {
					log("Omitting " + source + " as " + destination
				...
				if (fileName.endsWith(".class")) {
					...
					RewritingUtils.rewriteClassFile(source, transformer,
							destination);
				}
				...
				if (fileName.endsWith(".jar") || fileName.endsWith(".ear")
					...
					RewritingUtils.rewriteJar(new JarInputStream(
							new FileInputStream(source)), transformer,
