...
public final class AsmClassTransformer implements ResourceTransformer {
	...
	private byte[] transform(final ClassReader cr) {
		...
		cr.accept(
				new ContinuationClassAdapter(decorateClassVisitor(cw, true,
						null/* System.err */)), 0);
	...
	private ClassVisitor decorateClassVisitor(ClassVisitor visitor,
			final boolean check, final PrintStream dumpStream) {
		if (check) {
			visitor = new CheckClassAdapter(visitor);
			if (null != CHECK_DATA_FLOW) {
				try {
					// Currently CheckMethodAdapter throws error, so suppress
					// flow checks
		...
		if (null != dumpStream) {
			visitor = new TraceClassVisitor(visitor,
					new PrintWriter(dumpStream));
		}
	...
	static {
		...
		try {
			checkDataFlow = CheckClassAdapter.class
					.getDeclaredField("checkDataFlow");
