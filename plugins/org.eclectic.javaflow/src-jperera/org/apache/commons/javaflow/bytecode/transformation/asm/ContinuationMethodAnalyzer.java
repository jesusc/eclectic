...
public class ContinuationMethodAnalyzer extends MethodNode implements Opcodes {
	...
	void moveNew() throws AnalyzerException {
		...
		for (final Map.Entry<AbstractInsnNode, MethodInsnNode> e : movable
				.entrySet()) {
			...
			if (args.length == 1 && args[0].getSize() == 1) {
				...
				if (requireDup) {
					...
					updateMaxStack = updateMaxStack < 2 ? 2 : updateMaxStack; // a
																				// two
																				// extra
																				// slots
																				// for
			...
			if ((args.length == 1 && args[0].getSize() == 2)
					|| (args.length == 2 && args[0].getSize() == 1 && args[1]
							.getSize() == 1)) {
				final InsnList doNew = new InsnList();
				doNew.add(node1); // NEW
				if (requireDup) {
					...
					updateMaxStack = updateMaxStack < 1 ? 1 : updateMaxStack; // an
																				// extra
																				// slot
																				// for
																				// temp
